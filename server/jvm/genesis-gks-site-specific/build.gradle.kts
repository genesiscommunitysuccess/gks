    ...
    main {
        contents {
            ...
            into("site-specific/cfg") {
                from("${project.rootProject.projectDir}/genesis-gks-site-specific/src/main/resources/cfg")
                filter(
            ...
            into("site-specific/scripts") {
                from("${project.rootProject.projectDir}/genesis-gks-site-specific/src/main/resources/scripts")
                filter(
                    org.apache.tools.ant.filters.FixCrLfFilter::class,
                    "eol" to org.apache.tools.ant.filters.FixCrLfFilter.CrLf.newInstance("lf")
                )
            ...
            into("site-specific/data") {
                from("${project.rootProject.projectDir}/genesis-gks-site-specific/src/main/resources/data")
                filter(
                    org.apache.tools.ant.filters.FixCrLfFilter::class,
                    "eol" to org.apache.tools.ant.filters.FixCrLfFilter.CrLf.newInstance("lf")
                )
...
tasks {
    distZip {
        archiveBaseName.set("genesis-gks-site-specific")
...
publishing {
    publications {
        create<MavenPublication>("genesis-gksSiteSpecificDistribution") {
...
artifactory {
    ...
    val publicationNames = arrayOf("genesis-gksSiteSpecificDistribution")
    publish(

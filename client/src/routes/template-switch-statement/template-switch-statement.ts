...
const getTemplateForMode = (mode: TemplateMode): ViewTemplate  => {
  ...
  switch (mode) {
    case TemplateMode.MODE_1:
      return html`<h2>Mode 1</h2>`

    case TemplateMode.MODE_2:
      return html`<h2>Mode 2</h2>`

    case TemplateMode.MODE_3:
      return html`<h2>Mode 3</h2>`

    case TemplateMode.MODE_4:
      return html`<h2>Mode 4</h2>`
  }
}
...
@customElement({
  ...
  template: html<TemplateSwitchStatement>`
    <tutorial-container :route="${() => 'template-switch-statement'}">
      ...
      <div class="buttons">
          ${repeat(x => x.ribbonButtons, html<TemplateMode>`
              <zero-button 
                      @click="${(x, ctx: ExecutionContext<TemplateSwitchStatement>) => ctx.parent.setMode(x)}">
                  ${x => x}
              </zero-button>
          `)}
      </div>
      <div class="mode">
          ${x => getTemplateForMode(x.mode)}
      </div>
    </tutorial-container>
  `,
  styles: css`
